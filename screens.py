...
def home(draw, selected: int) -> None:
    header(draw, "Home")
    menu(draw, MENU_OPTIONS, selected)
    footer(draw, "DS4: X entra | O volta")


def send_screen(draw, status_text: str = "Pronto para enviar") -> None:
...
def receive_screen(draw, last_msg: str) -> None:
...
def system_screen(draw, radio_ok: bool, gamepad_name: str | None) -> None:
    header(draw, "Sistema")
    body_text(
        draw,
        f"NRF: {'OK' if radio_ok else 'FALHA'}",
        40,
        (0, 255, 255) if radio_ok else (255, 80, 80),
    )
